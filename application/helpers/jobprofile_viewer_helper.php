...
function generateViewer($mydiv, $mydept, $posisi, $atasan, $tujuanjabatan, $tgjwb){
    // viewer atasan
    if (empty($posisi['id_atasan1'])){
        $viewer_atasan = "<div class='col-md-9'><span class='badge badge-danger'>: Data Kosong</span></div>";
    } else {
        $viewer_atasan = "<div class='col-md-9': >: ".$atasan['position_name']."</div>";
    }

    // viewer tujuan jabatan
    if(empty($tujuanjabatan)){
        $viewer_tujab = "<div class='row ml-0 mb-2'>
            <div class='col-lg-12 view-tujuan alert alert-danger'>
                <i>Karyawan belum mengisi Tujuan Jabatan</i>
            </div>
        </div>";
    } else {
        $viewer_tujab = "<div class='row ml-1 mb-2'>
    ...
    // viewer tanggung jawab
    if(empty($tgjwb)){
        $viewer_tawab = "<div class='col-12 alert alert-danger'>
            <i>Karyawan belum mengisi Tanggung Jawab Utama, Aktivitas Utama & Indikator Kinerja</i>
        </div>";
    } else {
        // generate viewer table
        foreach ($tgjwb as $k => $t){
            $row_tawab[$k] = "<tr id='".$t['id_tgjwb']."'>
                <td>".$t['keterangan']."</td>
                <td>".$t['list_aktivitas']."</td>
                <td>".$t['list_pengukuran']."</td>
            </tr>";
        }

        $table_tawab = implode(" ", $row_tawab);

        // genereate viewer tawab
        $viewer_tawab = "<table id='tanggung-jawab' class='table table-bordered table-hover'>
    ...
    // viewer ruang lingkup
    if(empty($ruangl)){
        $viewer_rualing = "<div class='col-12 alert alert-danger'>
            <i>Karyawan belum mengisi Ruang Lingkup Jabatan</i>
        </div>";
    } else {
        $viewer_rualing = "<div class='col-12 view-ruang'>
            ".$ruangl['r_lingkup']."
        </div>";
    }
